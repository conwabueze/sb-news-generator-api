 ...
 */
const createChat = async (sbAuthKey, accessorIDs) => {
    const url = `https://app.staffbase.com/api/spaces/${accessorIDs[0]}/installations`;
...
export const chatInstallation = async (sbAuthKey, accessorIDs) => {
    ...
    const currEnvinstallations = await getInstallations(sbAuthKey, accessorIDs);
    ...
    if (!pluginIDs.includes('chat')) {
        const installChat = await createChat(sbAuthKey, accessorIDs);
